...
import { newCar, updateCurrentCar } from "../data-services";
...
export async function updateCar(
  ...
  // Validation (same rules as createCar)
  if (!brandName?.trim()) throw new Error("Brand name is required");
  if (!year || isNaN(year)) throw new Error("A valid year is required");
  if (!condition || !["New", "Used"].includes(condition)) {
    throw new Error("Condition is required");
  }
  if (condition === "Used" && (isNaN(mileage) || mileage < 0)) {
  ...
  try {
    ...
    console.error("updateCurrentCar failed:", error);
    throw new Error(
