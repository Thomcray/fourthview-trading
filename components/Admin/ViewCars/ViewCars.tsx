...
export default function ViewCars() {
  ...
  const [confirmCar, setConfirmCar] = useState<CarType | null>(null);
  ...
  const handleDelete = async (car: CarType) => {
    ...
    try {
        ...
        (old: { pages: { cars: CarType[]; total: number }[] } | undefined) => {
          ...
          return {
            ...
            pages: old.pages.map((page) => ({
              ...page,
              cars: page.cars.filter((c: CarType) => c.id !== car.id),
              total: page.total - 1,
            })),
          };
        },
      ...
      toast.success(`"${car.brandName}" deleted successfully`);
      setConfirmCar(null);
    } catch {
                ...
                {!hasNextPage && allCars.length > 0 && (
                  <div className="px-6 py-4 border-t border-gray-100 text-center">
                    <p className="text-sm text-gray-400">
                      End of results — {allCars.length} cars loaded
                    </p>
                  </div>
                )}
              </>
            ...
            >
              ...
              <p className="text-sm text-gray-600 mb-6">
                ...
                <span className="font-semibold text-gray-800">
                  {confirmCar.year} {confirmCar.brandName}
                </span>
                ? This will permanently remove the listing and its images from
                storage.
              </p>
